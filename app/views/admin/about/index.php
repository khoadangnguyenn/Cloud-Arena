...
<script>
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target && e.target.classList && e.target.classList.contains('remove-section')) {
            var s = e.target.closest('.section-item');
            if (s) s.remove();
        }
    });
    ...
    (function() {
        ...
        document.addEventListener('click', function(e) {
            if (e.target && e.target.id === 'add-section') {
                setTimeout(updateLegacyVisibility, 50);
            }
        });
        document.addEventListener('click', function(e) {
            if (e.target && e.target.classList && e.target.classList.contains('remove-section')) {
        ...
        if (checkbox) {
            checkbox.addEventListener('change', updateLegacyVisibility);
        }
        // initial state
...
<script>
    ...
    document.addEventListener('click', function(e) {
        if (e.target && e.target.classList && e.target.classList.contains('admin-thumb')) {
            ...
            if (main && fn) {
                main.src = '<?php echo URLROOT; ?>/uploads/' + fn;
            }
            if (hidden && fn) {
                hidden.value = fn;
            }
...
<script>
    ...
    (function() {
        ...
        // initialize
        try {
            vis.value = real.value || vis.value;
        } catch (e) {}
