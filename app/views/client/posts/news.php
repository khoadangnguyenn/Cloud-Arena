...
<div class="news-container max-w-[1400px] mx-auto px-6 py-12" id="news-section">
    <div class="news-header mb-16 text-center" data-aos="fade-down">
        ...
        <div class="search-and-filter mt-10 flex flex-col md:flex-row items-center justify-between gap-6 border-b border-white/10 pb-8">
            <div class="filter-group flex flex-wrap gap-3">
                ...
                <?php if (!empty($data['categories'])): ?>
                    <?php foreach ($data['categories'] as $cat): ?>
                        <a href="<?php echo URLROOT; ?>/posts?category=<?php echo htmlspecialchars($cat->slug); ?>"
                            class="filter-btn <?php echo $data['category'] == $cat->slug ? 'active' : ''; ?>">
            ...
            <form action="<?php echo URLROOT; ?>/posts" method="GET" class="search-wrapper-v2 relative w-full md:w-80">
                <input type="text" name="search" id="news-search" class="w-full bg-white/5 border border-white/10 rounded-full py-3 px-6 text-sm focus:outline-none focus:border-cyan-500 transition-all" placeholder="Tìm kiếm câu chuyện..." value="<?php echo htmlspecialchars($data['search']); ?>">
    ...
    <?php if (!empty($data['news'])): ?>
        ...
        <div class="flex flex-col lg:flex-row gap-12 mt-4">
            <div class="lg:w-3/4">
                <div class="news-river grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch" id="news-grid">
                    <?php
                    // If searching, show all. If not, skip first 3 (which are in Bento Box).
                    $remaining = !empty($data['search']) ? $data['news'] : array_slice($data['news'], 3);
                    foreach ($remaining as $index => $article):
                    ?>
                        <div class="news-card-v2 group flex flex-col h-full" data-aos="fade-up" data-aos-delay="<?php echo ($index % 3) * 50; ?>" data-title="<?php echo htmlspecialchars(strtolower($article->title)); ?>" data-category="<?php echo htmlspecialchars($article->category_slug ?? ''); ?>">
                            <div class="relative aspect-video rounded-2xl overflow-hidden mb-5 border border-white/5 flex-shrink-0">
                                <img src="<?php echo $article->thumbnail ? URLROOT . '/uploads/' . $article->thumbnail : 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80'; ?>" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" alt="<?php echo htmlspecialchars($article->title); ?>">
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-white/5 text-[10px] text-gray-500 uppercase tracking-widest mt-auto">
            ...
            <aside class="lg:w-1/4">
                <div class="sticky top-24">
                    ...
                    <div class="newsletter-card bg-gradient-to-br from-purple-900/40 to-blue-900/40 p-8 rounded-3xl border border-white/10">
                        ...
                        <div class="mt-4">
                            <a href="<?php echo URLROOT; ?>/users/register" class="w-full bg-cyan-500 text-white px-6 py-3 rounded-xl text-sm font-bold uppercase hover:bg-cyan-600 transition-all duration-300 flex items-center justify-center gap-2 group">
                                Tham gia ngay <i class="fa-solid fa-arrow-right group-hover:translate-x-1 transition-transform"></i>
                            </a>
                        </div>
                    </div>
                </div>
...
<style>
    ...
    .editorial-grid h2 a:hover,
    .editorial-grid h3 a:hover {
