            ...
            <?php if (isset($data['current_category']) && $data['current_category']): ?>
                ...
                <?php if (!empty($data['faqs'])): ?>
                    ...
                    <?php if (!empty($data['pagination'])): ?>
                        <div class="mt-6 px-4">
                            <div class="faq-pagination-wrapper"><?php echo $data['pagination']; ?></div>
                        </div>
                    <?php endif; ?>
                <?php else: ?>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            fetch(f.getAttribute('action') || '<?php echo URLROOT; ?>/pages/faqMessage', {
                    method: 'POST',
                    body: data,
                    headers: {
        ...
        if (btnHistory) {
            btnHistory.addEventListener('click', function() {
                ...
                fetch(url, {
                    ...
                    .then(r => r.json()).then(function(j) {
                        if (!j || !j.success) {
                            alert('Không lấy được lịch sử');
                            return;
                        }
                        var msgs = j.messages || [];
                        if (msgs.length === 0) {
                            convBox.innerHTML = '<div class="text-gray-400">Chưa có lịch sử.</div>';
                        ...
                        msgs.forEach(function(m) {
                            ...
                            if (m.reply) {
                                html += '<div style="margin-top:6px;padding:8px;background:#072018;border-radius:6px;color:#d1fae5">Trả lời: ' + nl2br(escapeHtml(m.reply)) + '<br><small style="color:#9ca3af">Bởi: ' + escapeHtml(m.reply_by || '') + ' · ' + escapeHtml(m.replied_at || '') + '</small></div>';
                            }
                            html += '</div>';
                        });
                        ...
                        alert('Lỗi mạng khi lấy lịch sử');
                    });
            });
        }
    });
...
<style>
    ...
    .faq-category-grid .cat-card:hover {
        transform: translateY(-8px) scale(1.02);
        box-shadow: 0 18px 40px rgba(2, 6, 23, 0.6)
    }

    .faq-category-grid .cat-card .cat-img {
        width: 100%;
        height: 110px;
        object-fit: cover;
        border-radius: 10px;
    ...
    .faq-category-grid .cat-card .text-sm {
        font-weight: 600;
        font-size: 15px;
        color: #e6eef8
    }

    .faq-category-grid .cat-card .cat-img+div {
        margin-top: 4px
    }
...
<style>
    ...
    .faq-pagination-wrapper nav.pagination {
        display: block
    }

    .faq-pagination-wrapper .pagination-list {
        display: flex;
        gap: 8px;
        align-items: center;
        list-style: none;
    ...
    .faq-pagination-wrapper .page-item {
        display: inline-flex
    }

    .faq-pagination-wrapper .page-link,
    .faq-pagination-wrapper .page-ellipsis {
    ...
    .faq-pagination-wrapper .page-link:hover {
        background: linear-gradient(90deg, #06b6d4, #9333ea);
        color: #fff;
