...
class Order {
    ...
    public function getLastFiveMonthRevenue() {
        $this->db->query("SELECT DATE_FORMAT(created_at, '%Y-%m') AS month_key, IFNULL(SUM(total_amount),0) AS revenue
                          FROM orders
    ...
    public function getMonthlyRevenue() {
        $this->db->query("SELECT IFNULL(SUM(total_amount),0) AS revenue
                          FROM orders
